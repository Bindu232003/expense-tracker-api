...
// GET: Fetch all expenses (R - Read)  <--- THIS WAS THE MISSING ROUTE!
    ...
    const categorySummary = await Expense.aggregate([
      {
        $group: {
          ...
          totalSpent: { $sum: '$amount' }, // Calculate the sum of 'amount' for each group
          count: { $sum: 1 }            // Count the number of expenses in each group
        }
      },
      {
        $sort: { totalSpent: -1 }      // Sort the results by totalSpent descending
      }
    ]);
...
// GET: Get total spending per month
